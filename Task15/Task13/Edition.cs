...
namespace Task13
{
    public class Edition : IComparable<Edition>
    {
        ...
        {
            var thisFirstAuthor = Authors.Split(',').First().Trim();
            var otherFirstAuthor = other.Authors.Split(',').First().Trim();

            int authorCompare = string.Compare(thisFirstAuthor, otherFirstAuthor, StringComparison.OrdinalIgnoreCase);
            if (authorCompare != 0)
